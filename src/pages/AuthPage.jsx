...
const AuthPage = () => {
  ...
  const handleVerify = async () => {
    ...
    setSubmitting(true);
    try {
      const password = phone; // phone number as password
      let firebaseUser;
      let isNewUser = false;
      ...
      if (isNewUser && firebaseUser) {
        ...
        await setDoc(doc(db, "users", firebaseUser.uid), {
          ...
          year: "Student",
          bio: "Campus Hustler | ITBM College",
          skills: [],
          hustleCoins: 0,
          totalEarnings: 0,
          totalHustles: 0,
      ...
      setVerified(true);
      // onAuthStateChanged in App.jsx will detect the login and switch to main app
    } catch (err) {
      console.error(err);
      if (
        ...
        setOtpError("This email is already registered with a different phone number.");
      } else {
  ...
  const inputCls = (hasError) =>
    "w-full pl-10 pr-4 py-3 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none " +
    (hasError
      ? "border-2 border-red-500/60 bg-red-500/5"
      : "border border-white/10 bg-white/5 focus:border-purple-500/70");
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="flex items-center justify-center gap-2.5 mb-8">
          ...
          <span
            ...
            style={{ background: "linear-gradient(135deg,#a855f7,#60a5fa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}
          >
            CAMPUS
          </span>
          ...
          >
            {verified ? (
              ...
              <>
                ...
                {showOtpHint && (
                  <div
                    className="mb-5 rounded-xl p-3 flex items-start justify-between gap-3"
                  ...
                  >
                    ...
                    <button type="button" onClick={() => setShowOtpHint(false)} className="text-gray-600 hover:text-gray-400 shrink-0 mt-0.5">
                      <X size={14} />
                    </button>
                  </div>
                )}
                ...
                <button
                  ...
                  style={{ opacity: otp.some((d) => !d) || submitting ? 0.5 : 1, cursor: otp.some((d) => !d) || submitting ? "not-allowed" : "pointer" }}
                >
                  {submitting ? "Verifying…" : <><CheckCircle2 size={16} /> Verify &amp; Enter</>}
                </button>
