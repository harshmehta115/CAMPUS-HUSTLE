...
import {
  ...
  TrendingUp, IndianRupee, Clock, ExternalLink, Save, X, Plus, Trash2
} from 'lucide-react';
...
const ProfilePage = () => {
  ...
  const { user, myCompleted, updateBio, updateSkills, addToast } = useApp();
    ...
    <div className="page-enter min-h-screen pt-20 pb-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative rounded-3xl overflow-hidden mb-6">
          ...
          <div className="px-6 pb-6" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderTop: 'none', borderRadius: '0 0 24px 24px' }}>
            <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 -mt-10 sm:-mt-12 mb-5">
              {/* Avatar */}
              <div
                className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl flex items-center justify-center text-2xl font-black text-white ring-4 ring-navy-950 shrink-0 z-10 select-none"
              ...
              <div className="flex-1 min-w-0 sm:pb-1">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      onClick={handleShare}
                      className="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-500 hover:text-white transition-all hover:border-white/20"
                      title="Share profile"
                    >
                      <Share2 size={15} />
                    </button>
                    <button
                      onClick={() => { setEditing(!editing); setBioInput(user.bio); }}
            ...
            {editing ? (
              <div className="mb-4 space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={handleSaveBio} className="btn-primary text-xs px-5 py-2 gap-1.5">
                    <Save size={13} /> Save Changes
                  </button>
                  <button onClick={handleCancelBio} className="btn-secondary text-xs px-5 py-2">
                    Cancel
                  </button>
                </div>
              </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Earned', value: `₹${(totalEarnings || user.totalEarnings).toLocaleString()}`, color: '#22c55e', icon: IndianRupee },
        ...
        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
          {[
            ...
            <button
              key={id}
              onClick={() => setTab(id)}
              className={`tab-btn shrink-0 ${tab === id ? 'active' : ''}`}
            >
              {label}
            </button>
          ))}
                ...
                <div>
                  ...
                  {Object.entries(user.social).map(([platform, handle]) => (
                    <div key={platform} className="flex items-center gap-3 mb-2">
                      ...
                      <button
                        onClick={() => addToast(`Opening ${platform}... (demo mode)`, 'info')}
            ...
            <div className="card lg:col-span-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { icon: '🎯', title: 'Perfect Score', desc: '5★ three times', earned: myCompleted.filter(h => h.rating === 5).length >= 3 },
                  { icon: '👑', title: 'Campus Pro', desc: '50+ hustles', earned: user.totalHustles >= 50 },
                  { icon: '💎', title: 'Elite Hustler', desc: '₹10k+ earned', earned: user.totalEarnings >= 10000 },
                  { icon: '🚀', title: 'Trending', desc: 'Featured hustle', earned: false },
                ].map((a) => (
                  <div
                    key={a.title}
                    className={`rounded-xl p-3 text-center transition-all ${a.earned ? '' : 'opacity-30 grayscale'}`}
                    style={{
                      background: a.earned ? 'rgba(168,85,247,0.12)' : 'rgba(255,255,255,0.03)',
                      border: a.earned ? '1px solid rgba(168,85,247,0.3)' : '1px solid rgba(255,255,255,0.05)'
                    }}
                  >
                ...
                {myCompleted.map((h) => (
                  <div key={h.id} className="card flex items-center justify-between gap-4 flex-wrap">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl bg-white/5 shrink-0">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-green-400 font-black text-xl">+₹{h.earnings}</p>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {h.review ? (
                            ...
                            <p className="text-gray-600 text-xs italic">No written review left.</p>
                          )}
                        </div>
                      </div>
                    </div>
